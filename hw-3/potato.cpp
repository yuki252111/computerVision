...
void thin(int**p, int height,int width,int** strucEle, int originX, int originY,int row,int col)//thin操作 
{
	//复制二维数组
	int** copy = new int*[height];
	for (int i = 0; i < height; i++)
...
void getSkeletion(int** p, int height, int width)  //骨架提取函数
{
	//先构造8个结构元素
	int originX, originY;
	int** p1 = new int*[3];
	...
	for (int count = 0; count < 100;count++) //重复进行100次
	{
		//第一个结构元素，用它进行thin操作
		p1[0][0] = 0; p1[0][1] = 0; p1[0][2] = 0;
		p1[1][0] = -1; p1[1][1] = 1; p1[1][2] = -1;
		p1[2][0] = 1; p1[2][1] = 1; p1[2][2] = 1;
		thin(p, height, width, p1, originX, originY, 3, 3);
		//第二个结构元素，用它进行thin操作
...
int main()
{
	...
	IplImage* skele = cvCreateImage(cvGetSize(rgb), rgb->depth, rgb->nChannels);
	cvCopy(rgb, skele);                         //复制rgb图像
	//将灰度读入二位数组
	...
	getSkeletion(p, height, width);              //进行骨架提取
	//对于是骨架的地方，把颜色设置为绿色
	for (int i = 0; i<height; i++)
	{
		ptr = (uchar*)skele->imageData + i*skele->widthStep;
		for (int j = 0; j<width; j++)
		{
			if (p[i][j] == 1)                  //判断是否为骨架
			{
